...
public final class URLUtil
{
     ...
     * for [WSTX-275].
     */
    private final static RegExp URI_WINDOWS_FILE_PATTERN = RegExp.compile("^file:///\\p{Alpha}|.*$");
    ...
    public static URI uriFromSystemId(final String sysId) throws IOException
    {
        ...
        try {
            if (sysId.indexOf('|', 0) > 0) {
                if (URI_WINDOWS_FILE_PATTERN.test(sysId)) {
        ...
        throws IOException
    {
        if ("file".equals(url.getProtocol())) {
            ...
            String host = url.getHost();
            if (host == null || host.length() == 0) {
                throw new UnsupportedOperationException("outputStreamFromURL " + url.getPath());
...
//        return f.toURI().toURL();
    ...
    */
    ...
    private static String cleanSystemId(final String sysId)
    {
        ...
        if (ix > 0 && URI_WINDOWS_FILE_PATTERN.test(sysId)) {
